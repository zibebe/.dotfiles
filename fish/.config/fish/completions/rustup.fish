...
complete -c rustup -n __fish_rustup_needs_command -f -a install -d 'Install or update the given toolchains, or by default the active toolchain'
complete -c rustup -n __fish_rustup_needs_command -f -a uninstall -d 'Uninstall the given toolchains'
complete -c rustup -n __fish_rustup_needs_command -f -a dump-testament -d 'Dump information about the build'
complete -c rustup -n __fish_rustup_needs_command -f -a toolchain -d 'Install, uninstall, or list toolchains'
complete -c rustup -n __fish_rustup_needs_command -f -a default -d 'Set the default toolchain'
complete -c rustup -n __fish_rustup_needs_command -f -a show -d 'Show the active and installed toolchains or profiles'
complete -c rustup -n __fish_rustup_needs_command -f -a update -d 'Update Rust toolchains and rustup'
complete -c rustup -n __fish_rustup_needs_command -f -a check -d 'Check for updates to Rust toolchains and rustup'
complete -c rustup -n __fish_rustup_needs_command -f -a target -d 'Modify a toolchain\'s supported targets'
complete -c rustup -n __fish_rustup_needs_command -f -a component -d 'Modify a toolchain\'s installed components'
complete -c rustup -n __fish_rustup_needs_command -f -a override -d 'Modify toolchain overrides for directories'
complete -c rustup -n __fish_rustup_needs_command -f -a run -d 'Run a command with an environment configured for a given toolchain'
complete -c rustup -n __fish_rustup_needs_command -f -a which -d 'Display which binary will be run for a given command'
complete -c rustup -n __fish_rustup_needs_command -f -a doc -d 'Open the documentation for the current toolchain'
complete -c rustup -n __fish_rustup_needs_command -f -a man -d 'View the man page for a given command'
complete -c rustup -n __fish_rustup_needs_command -f -a self -d 'Modify the rustup installation'
complete -c rustup -n __fish_rustup_needs_command -f -a set -d 'Alter rustup settings'
complete -c rustup -n __fish_rustup_needs_command -f -a completions -d 'Generate tab-completion scripts for your shell'
complete -c rustup -n __fish_rustup_needs_command -f -a help -d 'Print this message or the help of the given subcommand(s)'
